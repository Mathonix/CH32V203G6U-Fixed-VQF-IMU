...
#define NaN NAN
#define M_SQRT2f    1.41421356237309504880f   // M_SQRT2fs
...
SLOW_CODE static void vqf_fill_double(vqf_double_t *dst, size_t n, vqf_double_t val)
{
    for (size_t i = 0; i < n; i++) {
        dst[i] = val;
    }
...
SLOW_CODE static vqf_real_t gainFromTau(vqf_real_t tau, vqf_real_t Ts)
{
    // assert(Ts > 0);
...
SLOW_CODE static void filterCoeffs(vqf_real_t tau, vqf_real_t Ts, vqf_double_t outB[], vqf_double_t outA[])
{
    // assert(tau > 0);
    // assert(Ts > 0);
    // second order Butterworth filter based on https://stackoverflow.com/a/52764064
    vqf_double_t fc = (M_SQRT2f / (2.0f*M_PIf))/(vqf_double_t)(tau); // time constant of dampened, non-oscillating part of step response
...
void updateAcc(const vqf_real_t acc[3])
{
    ...
    // acos can be replaced by 2*arctan( sqrtf(1-x*x) / x )
...
void updateMag(const vqf_real_t mag[3])
{
    ...
    if (params.magDistRejectionEnabled) {
        ...
        // asin can be replace by 2*arctan(x / sqrtf(1-x*x))
        ...
        if (fabsf(state.magNormDip[0] - state.magRefNorm) < params.magNormTh*state.magRefNorm
                && fabsf(state.magNormDip[1] - state.magRefDip) < params.magDipTh*(vqf_real_t)(M_PIf/180.0)) {
        ...
        if (fabsf(state.magNormDip[0] - state.magCandidateNorm) < params.magNormTh*state.magCandidateNorm
                && fabsf(state.magNormDip[1] - state.magCandidateDip) < params.magDipTh*(vqf_real_t)(M_PIf/180.0)) {
...
vqf_real_t getBiasEstimate(vqf_real_t out[3])
{
    ...
    // sqrt can be replaced by arm_sqrt_f32 from CMSIS_DSP
    return sqrtf(P)*(vqf_real_t)(M_PIf/100.0/180.0);
}

void setBiasEstimate(vqf_real_t bias[3], vqf_real_t sigma)
{
    memcpy(state.bias, bias, 3U * sizeof(vqf_real_t));
...
{
    // sqrt can be replaced by arm_sqrt_f32 from CMSIS_DSP
    out[0] = sqrtf(state.restLastSquaredDeviations[0]) / (params.restThGyr*(vqf_real_t)(M_PIf/180.0));
...
void setRestBiasEstEnabled(bool enabled)
{
    ...
    vqf_fill_real(state.restLastSquaredDeviations, 2, 0.0f);
